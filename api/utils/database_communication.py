...
def update_correlation(db: Session, user_correlation: data_models.CorrelationModel):
...
def get_correlation(db: Session, user_id: int, x_data_type: str, y_data_type: str) -> data_models.CorrelationModel | None:
    db_correlation = db.query(sql_models.UserCorrelation).filter(
